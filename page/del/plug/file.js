	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
				...
				isUpload: true,
				browserUrl: shopxx.base + "/eshop/file/browser.jhtml",
				uploadUrl: shopxx.base + "/eshop/file/upload.jhtml",
				callback: null
			};
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = shopxx.base + "/eshop/resources/admin/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = shopxx.base + "/eshop/resources/admin/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
